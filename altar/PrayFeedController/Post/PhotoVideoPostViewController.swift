...
class PhotoVideoPostViewController: UIViewController, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    override func viewDidLoad() {
      ...
      //   userProfileImageView.bottomAnchor.constraint(equalTo: photoImageView.topAnchor, constant: -8).isActive = true
        ...
        textoIngresado.text = NSLocalizedString("Say something about this photo ...", comment: "")
        navigationItem.rightBarButtonItem = UIBarButtonItem(title: NSLocalizedString("Post", comment: ""), style: .plain, target: self, action: #selector(postImageAndText))
        ...
        let uploadTask = imageRef.putData(data!, metadata: nil) { (matadata, error) in
            ...
            imageRef.downloadURL(completion: { (url, error) in
                if let url = url {
                    ...
                    if self.textoIngresado.text == NSLocalizedString("Say something about this photo ...", comment: "")
                    {
